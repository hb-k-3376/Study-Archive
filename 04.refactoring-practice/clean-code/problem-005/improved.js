...
const reviewValidator = {
  create: {
    ...
    validator: (review, user) => {
      try {
        const order = this.validatePurchase(user, review);
        this.validateTimeLimit(order);
        return { success: true };
      } catch (error) {
        return {
          success: false,
  ...
  update: {
    ...
    validator: (review, user) => {
      try {
        this.validateOwnerShip(review, user);
        this.validateReviewCount(review);
